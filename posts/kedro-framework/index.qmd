...
If you want to learn how to use Kedro's more advanced features check out the official [documentation](https://docs.kedro.org/en/stable/).
...
We declare our `base` catalog by modifying `/conf/base/catalog.yml`. The base catalog will be inherited by all other Kedro environments you create. For example, if you create a `dev` environment you don't need to repeat the catalog entries that are in `/conf/base/catalog.yml` inside of `/conf/dev/catalog.yml`.
...
Wonderul, we are all set now with source control and mitigating the possibility of committing anything confidential to your repository.
...
Kedro will automatically assume that you want to run all your pipelines (or your default pipeline) but you can, quite easily, change the specified command in the generated docker file.
