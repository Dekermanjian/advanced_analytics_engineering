...
:::{.callout-caution}
Depending on what opperating system you are using you may need to install `psycopg2-binary` instead of `psycopg2`. In this post we are going to be running everything from inside containers that are running `Linux`. So we will be using `psycopg2-binary`.
:::
...
## Initialize MLFlow
Now we need to initialize `MLFlow` whenever we start a run of our pipeline. As a reminder we are going to do that using the `Kedro` hook we defined scaffolding for earlier.
...
1. Log divergences if any occurred during sampling.
...
The artifacts are visible/accessible via the `MLFlow` UI, but remember that these objects are stored in our MinIO service. You can access the storage directly on `localhost:9001` and locate the objects in our default bucket:
