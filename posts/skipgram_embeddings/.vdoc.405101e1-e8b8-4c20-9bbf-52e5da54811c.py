...
dataloader = tf.data.Dataset.from_tensor_slices((dataset[0], dataset[1])).shuffle(100).batch(batch_size)
model_rng = random.PRNGKey(0)
sgns_rng = random.PRNGKey(1)
model = SkipGramEmbeddings(vocab_size, embed_len)
sgns = SGNSLoss(vocabulary=vocabulary, id_counts=id_counts, key=rng)
