...
In previous posts we learned what embeddings are, why they are important and how they can be leveraged in practice. We got our hands dirty by implementing two of the earliest foundational methods GloVe and Skip-gram with negative sampling (SGNS).
...
We are going to compare how a random weights initialized ELMo model performs in comparison to our briefly pre-trained ELMo model. Below we initialize a random model, very much the same way we initialized a model for pre-training. We also load our pre-trained weights from our saved checkpoint.
...
2. An attention pooling layer with explicit padding masking to collapse our sequences into a single representation
3. A two-layer multilayer perceptron with dropout and ReLU activation yielding our logits
...
This was a long one. Good job making it all the way to the end!
 ...
 In this post we reviewed the differences between static and contextual embeddings, we delved into the math and architecture of recurrent neural networks, and we implemented ELMo practically from scratch in JAX. In addition to all of that, we also then fine-tuned ELMo to perform text classification and we showed the difference in having pre-trained embeddings versus random weights.
