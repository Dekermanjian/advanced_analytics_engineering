...
def train(training_dataset: pl.DataFrame, model_config: dict, sampler_config: dict) -> dict:
    ...
    data_ref = ray.put(training_dataset.to_pandas())

    @ray.remote(max_retries=1, num_cpus=4)
    ...
    trained_models = {}
    trained_models_ref = [train_model.remote(data_ref=data_ref, city=city) for city in training_dataset['city'].unique()]
    while len(trained_models_ref) > 0:
        ready, trained_models_ref = ray.wait(trained_models_ref, num_returns=1)
        try:
            # Here you can do other stuff while waiting for the rest of your models to finish training
