...
Let's reuse our $\textbf{A}$ from above and let's say our vector is
...
Notice that we are enforcing identifiability by inducing a sum to zero over the parameters across the classes and notice that we are computing our logits using a sparse matrix vector multiplication algorithm.
...
```{python}
#| tbl-cap: Most influential words for class comp.graphics
...
```{python}
#| tbl-cap: Most influential words for class sci.med
...
```{python}
#| tbl-cap: Most influential words for class sci.space
...
Notice because of our transposed coefficients in the model definition we now have the features first and the class second in the brackets.
...
```{python}
#| tbl-cap: Most influential words for class comp.graphics - Structured Covariance Model
...
```{python}
#| tbl-cap: Most influential words for class sci.med - Structured Covariance Model
...
```{python}
#| tbl-cap: Most influential words for class sci.space - Structured Covariance Model
#| label: tbl-cov-scispace

cor_summary_df.filter(regex=r"(?i)sci.space\]$", axis='index').sort_values("mean", ascending=False).head(10)
```

Let's inspect the same feature for the same two classes that we did earlier to check whether our structured covariance has appropriately dealt with IIA.
...
Looking at the `f1-scores`, seems like we have a slight improvement due to the additional information captured by the dependencies across the classes.
...
In this post we saw that TF-IDF embeddings can produce very informative features for tasks like text-classification. We also saw how we can handle features, like TF-IDF, that are sparse to improve computational efficiency.
