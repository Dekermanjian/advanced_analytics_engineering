...
```
Great! Now similarly to how we defined our nodes in our data processing pipeline we will do the same for our ML pipeline. We are going to need to create nodes that do the following:
...
We also need to update the `fit()` method for a couple of reasons. The first is because we implemented a `normalize_target` method and we need to propagate that into the `fit()` method which calls our `build_model()` method. Second, because the default `fit()` method defines additional sampler configurations that we don't want because we have already centralized all of our configurations to be handled by `Kedro`.
...
class TSModel(ModelBuilder):
    ...
    def _serializable_model_config(self) -> dict[str, int | float | dict]:
        """
        Converts non-serializable values from model_config to their serializable reversible equivalent.
...
We need to be sure to define catalog entries for our training output otherwise `Kedro` will designate our trained models as `MemoryDataset`'s. However, there is no built-in dataset in `Kedro` that knows how to store our model. So we need to define a custom `Kedro` dataset to accommodate our needs.

### Creating a Custom Kedro Dataset
Creating a custom dataset can be quite simple. All we need to do is inherit from `Kedro`'s `AbstractDataset` and define the `init`, `save`, `load`, and `describe` methods. In our case, the `ModelBuilder` class has `save` and `load` methods that accept a filepath and we will use those methods in our custom dataset:
